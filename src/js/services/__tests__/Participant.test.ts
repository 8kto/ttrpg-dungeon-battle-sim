...
describe('Participant', () => {
  ...
  describe('constructor', () => {
    it('initializes currentHp using strategy.calculateHp', () => {
      const participant = new Participant(mockChar, 'hero', mockStrategy, 0, 0, mockLogger)
    ...
    it('sets hasAttackLimit correctly based on maxAttacksPerChar', () => {
      const withLimit = new Participant(mockChar, 'hero', mockStrategy, 0, 2, mockLogger)
      expect(withLimit.hasAttackLimit).toBe(true)

      const withoutLimit = new Participant(mockChar, 'hero', mockStrategy, 0, 0, mockLogger)
  ...
  describe('shouldSkip', () => {
    it('skips when d100 roll <= bias', () => {
      ...
      const participant = new Participant(mockChar, 'hero', mockStrategy, 30, 0, mockLogger)
  ...
  describe('attack', () => {
    it('skips turn when shouldSkip is true', () => {
      ...
      const participant = new Participant(mockChar, 'hero', mockStrategy, 10, 0, mockLogger)
      participant.attack([], { selectTarget: jest.fn() })
      expect(mockLogger.log).toHaveBeenCalledWith('🫢 Hero skips turn')
    })
    ...
    it('attacks once per damage entry against valid targets', () => {
      ...
      const enemyA = new Participant(mockEnemyChar, 'enemy', mockStrategy, 0, 0, mockLogger)
      const enemyB = new Participant({ ...mockEnemyChar, name: 'Enemy B' }, 'enemy', mockStrategy, 0, 0, mockLogger)
  ...
  describe('performOneAttack', () => {
    it('deals damage and logs hit on successful attack', () => {
      const attacker = new Participant(mockChar, 'hero', mockStrategy, 0, 0, mockLogger)
      const target = new Participant(mockEnemyChar, 'enemy', mockStrategy, 0, 0, mockLogger)
      jest.spyOn(attacker, 'rollToHit').mockReturnValue(20) // Hit

      attacker.performOneAttack(target, '1d6')
      expect(target.currentHp).toBe(5)
      expect(mockLogger.log).toHaveBeenCalledWith('🗡️ Hero attacks Enemy, damage: 5')
    })
    ...
    it('logs miss and no damage on unsuccessful attack', () => {
      ...
      expect(mockLogger.log).toHaveBeenCalledWith('🛡️ Hero misses Enemy')
    })
  })

  describe('isValidTarget', () => {
    it('returns false when dead or attack limit exhausted', () => {
      const dead = new Participant(mockChar, 'hero', mockStrategy, 0, 0, mockLogger)
      dead.currentHp = 0
      expect(dead.isValidTarget()).toBe(false)

      const exhausted = new Participant(mockChar, 'hero', mockStrategy, 0, 1, mockLogger)
      const attacker = new Participant(mockEnemyChar, 'enemy', mockStrategy, 0, 0, mockLogger)
  ...
  describe('resetAttackLimit', () => {
    it('resets maxAttacksRemaining to initial value', () => {
      const participant = new Participant(mockChar, 'hero', mockStrategy, 0, 3, mockLogger)
      const attacker = new Participant(mockEnemyChar, 'enemy', mockStrategy, 0, 0, mockLogger)
